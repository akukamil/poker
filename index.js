...
chat = {
	...
	async chat_updated(data) {		
		...
		//выбираем свободное или самое старое сообщение
		let rec = this.get_oldest_or_free_msg();
	...
	async open_keyboard() {
		...
		if (fb[0] === 'sent') {			
			...
			const msg_index=irnd(0,50);
			console.log('send message',msg_index)
			await firebase.database().ref('chat/'+msg_index).set({uid:my_data.uid,name:my_data.name,msg:fb[1], tm:firebase.database.ServerValue.TIMESTAMP, hash:hash});
		}		
	},
...
lobby={
	...
	header_move(){
		...
		objects.lobby_header.x-=1;
		if (objects.lobby_header.x<-750)
			objects.lobby_header.x=800;
